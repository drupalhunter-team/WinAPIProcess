...
PWSTR GetCMDLine(DWORD pid, LogFile *_log)
{
	...
	if (wow)
	{
		...
		if (err != 0)
		{
			_log->LogMessage("NtWow64QueryInformationProcess64 failed");
		...
		if (err != 0)
		{
			_log->LogMessage("NtWow64ReadVirtualMemory64 PEB failed");
			CloseHandle(hProcess);
			throw MsgException("FAIL");;
		}
		...
		if (err != 0)
		{
			_log->LogMessage("NtWow64ReadVirtualMemory64 Parameters failed");
			CloseHandle(hProcess);
			throw MsgException("FAIL");;
		}
	...
	else
	{
		...
		if (err != 0)
		{
			_log->LogMessage("NtQueryInformationProcess failed");
			CloseHandle(hProcess);
			throw MsgException("FAIL");;
		}
		...
		{
			_log->LogMessage("ReadProcessMemory PEB failed");
			CloseHandle(hProcess);
			throw MsgException("FAIL");;
		}
		...
		{
			_log->LogMessage("ReadProcessMemory Parameters failed");
			CloseHandle(hProcess);
			throw MsgException("FAIL");;
		}
