...
ProcessManager::ProcessManager(std::wstring cmdline)
{
	...
	_log = NULL;
	_app_status = "Stopped";
	_log = new LogFile("logfile.txt", false);
	_cmdline = cmdline;
...
void ProcessManager::_startProcess(){
	...
	_log->LogMessage("Trying to start process");
		...
		)
	{
		...
		_log->LogMessage(strStream.str());
		throw MsgException(strStream.str());
	}
	else {
		_log->LogMessage("Process started succesfully");
	}
}

void ProcessManager::_stopProcess(){
	_log->LogMessage("Trying to shutdown the process");
	if (_exeTrd != NULL && _trdalive){
		_trdalive = false;
		_exeTrd->join();
		delete _exeTrd;
		_exeTrd = NULL;
		_log->LogMessage("Process terminated");
		_app_status = "Stopped";
	}
	else {
		_log->LogMessage("Process is not running thus it wasn't stopped");
	}
}
